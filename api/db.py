...
SELECT
    torrent_name AS name,
    ROUND(delta_bytes / (1024.0 * 1024.0 * 1024.0), 2) AS uploaded_gb
FROM deltas
WHERE delta_bytes IS NOT NULL
  AND delta_bytes > 0
  AND day = ?
...
async def get_by_day(date: str) -> list[dict]:
    async with aiosqlite.connect(DB_PATH) as conn:
        conn.row_factory = aiosqlite.Row
        async with conn.execute(_BY_DAY_QUERY, (date,)) as cursor:
            rows = await cursor.fetchall()
    return [{"name": row["name"], "uploaded_gb": row["uploaded_gb"]} for row in rows]
